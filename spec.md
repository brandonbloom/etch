...
| Files | `create`, `replace`, `delete`, `move`, `copy` | File-level verbs use explicit etch names rather than shell primitive names. Signatures are `create <path> <content>`, `replace <path> <content>`, `delete <path>`, `move <src> <dst>`, and `copy <src> <dst>`. `create` creates missing paths, no-ops when an existing path already has the same content, and fails if an existing path has different content. `replace` requires an existing regular file and ensures its entire content equals the supplied content. `copy` fails if the destination exists in the transaction base. `delete` is idempotent when the path is absent from the transaction base. `move` fails if the source is absent or destination exists in the transaction base. |
...
Tables are ordered rows and named columns of string cells. Markdown pipe tables and CSV use the same logical model. Markdown table rewrites may re-render the addressed table, but they must preserve surrounding Markdown bytes.
